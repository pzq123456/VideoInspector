"""
证据帧采集：tee 分支 → appsink → BufferRetriever 缓存最新原始帧

SafetyProbe（BatchMetadataOperator）只拿得到 batch 元数据、拿不到像素，
因此告警证据帧必须另走一条帧分支。本模块在 nvosd **前**插入：

    tee → queue → nvvideoconvert → capsfilter(NVMM RGB) → appsink

appsink 的 FrameCaptureRetriever.consume() 把最新一帧转成 BGR numpy 缓存进
FrameCache（{source_id: frame}）。告警触发时探针从缓存取帧作为 snapshot 交给
AlertManager，由 executor 线程自定义渲染（只画人物框 + 违规标签）→ JPEG →
base64 → webhook（见 server/alert/manager.py::draw_alert_boxes）。
...
线程模型（与 alert/manager.py 的 fire-and-forget 分工一致）：
  ...
  - executor 线程   ：_build_and_send 读快照并原地画框——此时该数组已从缓存
                      换出（或本就是本次证据帧），画框不会污染后续帧。
...
class FrameCaptureRetriever(BufferRetriever):
    """appsink 接收器：把最新一帧转成 BGR numpy 缓存，立即放行。
...
def add_evidence_capture(pipeline, cache: FrameCache, gpu_id: int = 0) -> str:
    ...
    Returns:
        tee 元素名，主链需把 vest 的 src 连到它，再分两路：
        tee → (osd → rtspout | fakesink)  与  tee → 捕获分支（本函数已连好）。
    """
